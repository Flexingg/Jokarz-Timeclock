...
fun EditActiveTimerDialog(
    ...
    var textVal by remember { mutableStateOf(sdf.format(Date(currentStartMs))) }
    var error by remember { mutableStateOf<String?>(null) }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Edit Active Start Time", fontWeight = FontWeight.Bold) },
        text = {
            Column {
                OutlinedTextField(
                    value = textVal,
        ...
        confirmButton = {
            Button(
                onClick = {
                    try {
                        val parsed = sdf.parse(textVal)
                        if (parsed != null) {
                            if (parsed.time > System.currentTimeMillis()) {
                        ...
                        error = "Invalid format: ${e.message}"
                    }
                }
            ) {
                Text("Save")
...
fun EditSessionDialog(
    ...
    val sdf = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.US)
    var startText by remember { mutableStateOf(sdf.format(Date(session.start))) }
    var endText by remember { mutableStateOf(sdf.format(Date(session.end))) }
    var noteText by remember { mutableStateOf(session.note) }
    var error by remember { mutableStateOf<String?>(null) }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Edit Completed Session", fontWeight = FontWeight.Bold) },
        text = {
        ...
        confirmButton = {
            Button(
                onClick = {
                    try {
                        val dStart = sdf.parse(startText)
                        val dEnd = sdf.parse(endText)
                        if (dStart != null && dEnd != null) {
                            if (dStart.time >= dEnd.time) {
                                error = "Start time must be before end time."
                            } else {
                                onSave(dStart.time, dEnd.time, noteText.trim())
                            }
                        } else {
...
fun AddManualShiftDialog(
    ...
    var startText by remember { mutableStateOf(sdf.format(Date(defaultStart))) }
    var endText by remember { mutableStateOf(sdf.format(Date(now))) }
...
fun PtoManagementDialog(
    ...
    var dateText by remember { mutableStateOf(sdfDate.format(Date())) }
        ...
        text = {
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                ...
                Button(
                    onClick = {
                        try {
                            val parsedDate = sdfDate.parse(dateText)
                            val hours = hoursText.toDoubleOrNull() ?: 0.0
                            if (parsedDate != null && hours > 0.0) {
                                onAddPto(parsedDate.time, hours, selectedType, noteText.trim())
                                noteText = ""
                            } else {
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("App Settings & Rules", fontWeight = FontWeight.Bold)
            }
        },
        text = {
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Weekly Analytics & Trends", fontWeight = FontWeight.Bold)
            }
        },
        text = {
            Column {
